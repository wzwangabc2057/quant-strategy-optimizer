...
v3 激进版 - 非线性倾斜
================================================================================
在v2基础上，增加非线性权重倾斜和市场环境检测。
结果: R4 25.08%, R5 25.95%, Sharpe 1.36/1.28
================================================================================
"""
import pandas as pd
import numpy as np
...
def detect_market_regime(price_series: pd.Series) -> Tuple[str, float, float]:
    ...
    # 牛市
    if ret_60 > 0.10 and trend > 0.02:
        return 'bull', 1.3, 1.2

    # 熊市
    if ret_60 < -0.10 or trend < -0.02:
        return 'bear', 0.6, 0.8
...
def apply_nonlinear_weight_tilt(weights: pd.DataFrame,
    ...
    portfolio = portfolio.merge(scores[['code', 'composite_score']], on='code', how='left')
    portfolio = portfolio.fillna({'composite_score': 50})
        ...
        for i in range(1, len(dates)):
            ...
            if date in rebalance_dates and i > 1:
                ...
                enhanced_portfolio = apply_nonlinear_weight_tilt(
                    portfolio, scored_df,
                    tilt_strength=tilt_mult,
                    max_weight=0.08 * weight_mult
                )
